            ...
            if (file) {
                if (file[KEY_PATH].indexOf('//usr/') !== -1) {
                    uni.getFileInfo({
                        ...
                        fail(error) {
                            console.error(
                                `base64 file broken, ${JSON.stringify(error)}`
                            );
                            transformBase64File(url, lru).then(
                                (path) => {
                                    resolve(path);
                    ...
                    uni.getSavedFileInfo({
                        ...
                        fail: (error) => {
                            console.error(
                                `the file is broken, redownload it, ${JSON.stringify(
                                    error
                                )}`
                            );
                            downloadFile(url, lru).then(
...
function transformBase64File(base64data: string, lru: string) {
    return new Promise((resolve, reject) => {
        const [, format, bodyData] =
            ...
            success() {
                uni.getFileInfo({
                    ...
                    fail: (error) => {
                        ...
                        console.error(
                            `getFileInfo ${path} failed, ${JSON.stringify(
                                error
                            )}`
                        );
                        resolve(path);
...
function downloadFile(url: string, lru: any) {
    return new Promise((resolve, reject) => {
        const downloader = url.startsWith('cloud://')
        ...
        downloader({
            ...
            success: function (res: any) {
                if (res.statusCode !== 200) {
                    console.error(
                        `downloadFile ${url} failed res.statusCode is not 200`
                    );
                    reject();
                    return;
                }
    ...
    uni.removeStorage({
        ...
        success: () => {
            uni.getSavedFileList({
                ...
                fail: (getError) => {
                    console.error(
                        `getSavedFileList failed, ${JSON.stringify(getError)}`
                    );
                }
            });
        }
...
function doLru(size: any) {
    if (size > MAX_SPACE_IN_B) {
        return Promise.reject();
    }
    return new Promise((resolve, reject) => {
        let totalSize = savedFiles[KEY_TOTAL_SIZE]
            ? savedFiles[KEY_TOTAL_SIZE]
            : 0;
        ...
        uni.setStorage({
            ...
            fail: (error) => {
                console.error(
                    `doLru setStorage failed, ${JSON.stringify(error)}`
                );
                reject();
            }
        });
...
function removeFiles(pathsShouldDelete: any) {
    for (const pathDel of pathsShouldDelete) {
        ...
        if (delPath.indexOf('//usr/') !== -1) {
            uni.getFileSystemManager().unlink({
                ...
                fail(error) {
                    console.error(
                        `removeSavedFile ${pathDel} failed, ${JSON.stringify(
                            error
                        )}`
                    );
                }
