...
import GD from './gradient.js';
import './string-polyfill.js';

export const penCache: any = {
    // 用于存储带 id 的 view 的 rect 信息
    viewRect: {},
    textLines: {}
};
export const clearPenCache = (id: any) => {
    if (id) {
...
export default class Painter {
    ...
    _background() {
        this.ctx.save();
        const { width, height }: any = this.style;
    ...
    _border({
        ...
        if (borderRadius) {
            ...
            if (border.length === 4) {
                r1 = Math.min(
                    border[0].toPx(false, minSize),
                    width / 2,
                    height / 2
                );
                r2 = Math.min(
                    border[1].toPx(false, minSize),
                    width / 2,
                    height / 2
                );
                r3 = Math.min(
                    border[2].toPx(false, minSize),
                    width / 2,
                    height / 2
                );
                r4 = Math.min(
                    border[3].toPx(false, minSize),
                    width / 2,
                    height / 2
                );
            } else {
     ...
     */
    _doClip(borderRadius: any, width: any, height: any, borderStyle: any) {
    ...
    _doBorder(view: any, width: any, height: any) {
        ...
        const { borderRadius, borderWidth, borderColor, borderStyle } =
            view.css;
        if (!borderWidth) {
            return;
        }
    ...
    _preProcess(view: any, notClip: any) {
        ...
        switch (view.type) {
            ...
            case 'text': {
                ...
                for (let i = 0; i < textArray.length; ++i) {
                    ...
                    const minWidth =
                        view.css.fontSize.toPx() + paddings[1] + paddings[3];
        ...
        if (view.css && view.css.right) {
            if (typeof view.css.right === 'string') {
                x =
                    this.style.width -
                    view.css.right.toPx(true, this.style.width);
            } else {
                // 可以用数组方式，把文字长度计算进去
                // [right, 文字id, 乘数（默认 1）]
    ...
    _doPaddings(view: any) {
        const { padding }: any = view.css ? view.css : {};
        let pd = [0, 0, 0, 0];
        if (padding) {
    ...
    _doBackground(view: any) {
        this.ctx.save();
        const { width: rawWidth, height: rawHeight }: any = this._preProcess(
            view,
            true
        );
    ...
    _drawQRCode(view: any) {
        this.ctx.save();
        const { width, height }: any = this._preProcess(view);
        QR.api.draw(
            view.content,
            this.ctx,
            -width / 2,
    ...
    _fillAbsInlineText(view: any) {
        ...
        this.ctx.save();
        const { width, height, extra }: any = this._preProcess(
            view,
    ...
    _fillAbsText(view) {
        ...
        if (view.id && penCache.textLines[view.id]) {
            ...
            for (let j = 0; j < textArray.length; ++j) {
                const preLineLength = Math.ceil(
                    textArray[j].length / linesArray[j]
                );
                let start = 0;
                let alreadyCount = 0;
