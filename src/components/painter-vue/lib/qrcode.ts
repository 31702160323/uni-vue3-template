...
var version, width: any, neccblk1, neccblk2, datablkw, eccblkwid;
var ecclevel = 2;
...
function appendrs(data: any, dlen: any, ecbuf: any, eclen: any) {
    ...
    for (i = 0; i < dlen; i++) {
        ...
        strinbuf[ecbuf + eclen - 1] =
            fb == 255 ? 0 : gexp[modnn(fb + genpoly[0])];
    }
}
...
function genframe(instring: any) {
    ...
    for (i = 0; i < datablkw; i++) {
        ...
        for (j = 0; j < neccblk2; j++)
            eccbuf[y++] =
                strinbuf[neccblk1 * datablkw + i + j * (datablkw + 1)];
    }
    for (j = 0; j < neccblk2; j++)
...
var api = {
    ...
    getFrame: function (string: string) {
        return genframe(string);
    },
    //这里的utf16to8(str)是对Text中的字符串进行转码，让其支持中文
    utf16to8: function (str: string) {
        var out, i, len, c;
