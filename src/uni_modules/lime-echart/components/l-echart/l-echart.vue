...
export default {
    ...
    created() {
        ...
        // #ifdef MP-ALIPAY
        this.use2dCanvas =
            this.type === '2d' && compareVersion(my.SDKVersion, '2.7.0') >= 0;
        // #endif
    },
    methods: {
        // #ifdef APP-NVUE
        ...
        setOption() {
            ...
            // #ifdef APP-NVUE
            this.$refs.webview.evalJs(
                `setOption(${JSON.stringify(arguments)})`
            );
            // #endif
        },
        showLoading() {
        ...
        async init(echarts, callback) {
            ...
            if (find) {
                ...
                callback(this.chart);
                this.$refs.webview.evalJs(
                    `init(${JSON.stringify(find.toString())}, ${JSON.stringify(
                        this.roptions
                    )})`
                );
            } else {
        ...
        touchMove(e) {
            if (
                this.chart &&
                (e.touches.length > 0 || (e.type == 'mousemove' && this.isDown))
            ) {
                const handler = this.chart.getZr().handler;
                dispatch.call(
                    handler,
                    'mousemove',
                    e.touches[0] || this.getRelative(e)
                );
        ...
        touchEnd(e) {
            ...
            if (this.chart) {
                const touch = e.changedTouches
                    ? e.changedTouches[0] || this.getRelative(e)
                    : {};
                const handler = this.chart.getZr().handler;
